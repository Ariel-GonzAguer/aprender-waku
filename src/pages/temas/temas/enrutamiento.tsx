...
export default function Enrutamiento() {
  ...
  return (
    <>
      ...
      <section className='text-center my-6 w-[90%] max-w-6xl mx-auto text-balance leading-relaxed'>
        <h2>{data.titulo}</h2>
        ...
        <p>Cada archivo `.tsx` dentro de la carpeta `src/pages` se convertirá una ruta de nuestra aplicación. Así de fácil.</p>

        <p>Páginas y layouts pueden crearse con archivos `.tsx` que contengan dos exportaciones: una función `default` para el componente React, y una función llamada `getConfig` que retorna un objeto de configuración para definir el tipo de renderizado y otras opciones.</p>


        <p>Como vimos en el tema anterior, Waku soporta dos tipos de renderizado: SSG (estático) y SSR (dinámico).
          Layouts, páginas y slices (se verán más adelante) son estáticos por defecto (SSG), mientras los handlers de API son dinámicos (SSR) por defecto.</p>
